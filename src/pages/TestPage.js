...
const TestPage = () => {
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (event) => {
            ...
            if (event.key === "g" || event.key === "i") {
                keySequenceRef.current += event.key;

                if (keySequenceRef.current === "gi") {
                    // Focus the TextField when "gi" is pressed
                    textFieldRef.current?.focus();
                    keySequenceRef.current = ""; // Reset key sequence
                }

                // Clear key sequence after 1 second if the combo isn't completed
        ...
        return () => {
            ...
            clearTimeout(keyTimeout); // Clear timeout when component unmounts
        };
    }, [openResultsDialog]);
